...
function varNumToDatArray($varNum, $include_historic = false) {
	...
	if($varNum < count($types)) {
		if (!array_key_exists($varName, $CACHE_DAT)) {
			$CACHE_DAT[$varName] = unserialize(file_get_contents(CACHE_ROOT . "serialised_dat_$varNum.txt"));
		}
	} else {
		if (!array_key_exists($varName, $CACHE_DAT)) {
			$idx = $varNum - $types_all['sunhr'];
			$CACHE_DAT[$varName] = unserialize(file_get_contents(CACHE_ROOT . "serialised_datm_$idx.txt"));
		}
	}
	...
	if($include_historic !== false && $start_year_all[$varNum] < 2009) {
		if($include_historic < 2009) {
			...
			if (!array_key_exists($varName, $CACHE_DAT_HIST)) {
				$CACHE_DAT_HIST[$varName] = unserialize(file_get_contents(CACHE_ROOT."serialised_historical_$varName.txt"));
			}
...
function write_datm($sunhrs) {
	...
	if($pond_temp === null) {
		$NOW = unserialize(file_get_contents(CACHE_ROOT . 'serialised_datNow.txt'));
