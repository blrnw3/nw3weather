<?php
/**
 * Build monthly hour-sampled logs from daily minute logs.
 * Invoked by cron_main at midnight.
 */
...
$Eday = isset($_GET['Eday']) ? intval($_GET['Eday']) : Date::$day_yest;
$Emonth = isset($_GET['Emonth']) ? intval($_GET['Emonth']) : Date::$mon_yest;
$Eyear = isset($_GET['Eyear']) ? intval($_GET['Eyear']) : Date::$yr_yest;

$Fday = isset($_GET['Fday']) ? intval($_GET['Fday']) : 1;
$Fmonth = isset($_GET['Fmonth']) ? intval($_GET['Fmonth']) : Date::$mon_yest;
$Fyear = isset($_GET['Fyear']) ? intval($_GET['Fyear']) : Date::$yr_yest;

$format = isset($_GET['format']) ? $_GET['format'] : 'txt';

$names = array(
	"Hour", "Minute", "Day", "10-min wind speed / mph", "Max Gust / mph",
