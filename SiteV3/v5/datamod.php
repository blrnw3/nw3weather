...
$types = array_flip($typesOriginal);
$dataNums = array(
...
$manualNums = array(8,9,10,-6,-6,-6,-6,-6,-6,-6,-6,-6,4,-6);
...
if (isset($_POST['pwd'])) {
	if ($_POST['pwd'] == 'datachanges') {
		$dailyLimit = min(count($modLine) - 2, count($typesOriginal));
		for ($i = 0; $i < $dailyLimit; $i++) {
			$name = $typesOriginal[$i];
		...
		for ($i = 0; $i < count($typesManual) && $i < count($modLineM); $i++) {
			$name = $typesManual[$i];
			if (isset($_POST[$name]) && $_POST[$name] != '') {
	...
	<form method="post" action="">
		<div class="table-main">
			<table class="table1" cellpadding="4">
				<tbody>
				<?php
				$dailyRows = min(count($modLine), count($typesOriginal));
				for ($i = 0; $i < $dailyRows; $i++) {
					$rowClass = ($i % 2 === 0) ? 'rowlight' : 'rowdark';
					$cssNum = $dataNums[$i] + 10;
					...
					echo '<tr class="' . $rowClass . '"><td class="td' . $cssNum . 'C">'
						. dataModEscape($typesOriginal[$i]) . '</td><td class="td' . $cssNum . 'C"' . $flagStyle . '>'
						. dataModEscape(isset($modLine[$i]) ? $modLine[$i] : '') . '</td><td><input type="text" name="'
						. dataModEscape($typesOriginal[$i]) . '" /></td><td class="td' . $cssNum . 'C">'
						. dataModEscape(isset($modLineT[$i]) ? $modLineT[$i] : '') . '</td><td><input type="text" name="'
						. dataModEscape($typesOriginal[$i]) . 't" /></td>';
					if ($i === 0) {
		...
		<div class="table-main">
			<table class="table1" cellpadding="5">
				...
				$manualRows = min(count($modLineM), count($typesManual));
				for ($i = 0; $i < $manualRows; $i++) {
					$rowClass = ($i % 2 === 0) ? 'rowlight' : 'rowdark';
					$cssNum = $manualNums[$i] + 10;
					$valueAttribute = $typesManual[$i] === 'comms'
						? ' value="' . dataModEscape($commentDefault) . '"' : '';
					echo '<tr class="' . $rowClass . '"><td class="td' . $cssNum . 'C">'
						. dataModEscape($typesManual[$i]) . '</td><td class="td' . $cssNum . 'C">'
						. dataModEscape($modLineM[$i]) . '</td><td><input type="text"' . $valueAttribute
						. ' name="' . dataModEscape($typesManual[$i]) . '" /></td>';
					if ($i === 0) {
