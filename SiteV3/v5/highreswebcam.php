...
Page::init([
	"fileNum" => 2,
	"title" => "High res Webcam Summary / Archive",
...
$cond1 = $sproc > Date::mkdate(3, 20, 2012);
...
$choseng = $chosens = $chosenh = $chosend = $chosena = '';
$height_mult = 0.75;
$camtype = isset($_GET['camtype']) ? $_GET['camtype'] : '';
if ($camtype == "gnd") {
	$cam_type = "gnd"; $choseng = 'checked="checked"';
} else if ($camtype == "sky") {
	$cam_type = "sky"; $chosens = 'checked="checked"';
} else {
	$cam_type = "hik"; $chosenh = 'checked="checked"'; $height_mult = 0.666;
}
if (isset($_GET['light']) && $_GET['light'] == "day") { $light = "day"; $chosend = 'checked="checked"'; } else { $light = "all"; $chosena = 'checked="checked"'; }
if (isset($_GET['width']) && isset($widths[(int)$_GET['width']])) { $width_opt = (int)$_GET['width']; } else { $width_opt = 3; }
...
if ($sproc <= Date::mkdate(9, 24, 2016)) {
	echo "<b>NB:</b> Before 24th Sep 2016, archive images are only available at 3hr intervals. <br />"
...
if ($pre_imgs) hrw_cam_table($pre_imgs, "Yesterday", -1);
hrw_cam_table($post_imgs, ($is_today) ? "Today" : $datedescrip, 0);

function hrw_cam_table($imgs, $heading, $day_offset) {
	global $sproc, $font_size, $qrand, $width_opt, $cam_type, $width, $height;
	...
	foreach ($img_rows as $null => $img_row) {
		...
		foreach ($img_row as $img) {
			...
			if (file_exists(Site::CAM_ROOT . $src)) {
