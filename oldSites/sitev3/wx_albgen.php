...
// Album data moved from photos/albums/ to albums/ at some point; accept either.
$albumDir = is_file(ROOT . 'photos/albums/wx_alb1.php') ? ROOT . 'photos/albums/' : ROOT . 'albums/';
require $albumDir . 'albInfo.php';

if(isset($_GET['albnum'])) {
	$_SESSION['albnum'] = (int) $_GET['albnum'];
}
$albnum = $_SESSION['albnum'];
if($albnum <= count($wx_titles) && $albnum > 0) { //input validation
	require $albumDir . 'wx_alb'.$albnum.'.php';
} else {
	die('Naughty! Album not in range');
}


$imgref = '/photos/'. $wx_refs[$albnum-1] . '/';
$title = $wx_titles[$albnum-1];
...
$htmlMETA_NAME = 'Weather Station Photography - '. $title;
...
if (!isset($_GET['view'])) {
	...
	if ($_GET['view'] == 'Full') { // All photos displayed
		...
		if ($num != 1) {
			echo '<a href="wx_albgen.php?view=Strip&img=1#start" title="First Photo">';
		} echo '&lt;&lt; START';
		if ($num != 1) {
			echo '</a>';
		} echo '</td>';
		echo '<td width="15%" align="left">';
		if ($num != 1) {
			echo '<a href="wx_albgen.php?view=Strip&img=', $num - 1, '#start" title="Previous Photo">';
		} echo '&lt; Previous';
		if ($num != 1) {
			echo '</a>';
		} echo '</td>';
		echo '<td width="40%" align="center"><a href="wx_albgen.php#main" title="Return to thumbnails">Album Overview</a></td>';
		echo '<td width="15%" align="right">';
		if ($num != $imgnum) {
			echo '<a href="wx_albgen.php?view=Strip&img=', $num + 1, '#start" title="Next Photo">';
		} echo 'Next &gt;';
		if ($num != $imgnum) {
			echo '</a>';
		} echo '</td>';
		echo '<td width="15%" align="right">';
		if ($num != $imgnum) {
			echo '<a href="wx_albgen.php?view=Strip&img=', $imgnum, '#start" title="Last Photo">';
		} echo 'END &gt;&gt;';
		if ($num != $imgnum) {
			echo '</a>';
		} echo '</td>';
		...
		if ($num == 1) {
			...
			for ($i = 2; $i <= 5; $i++) {
				echo '<td width="19%"><a href="wx_albgen.php?view=Strip&img=', $i, '#start"><img width="96%" src="', $imgref, $i, 's.jpg" title="', $imgdescrip[$i - 1], '" alt="Photo ', $i, '..." /></a></td>';
			}
		} elseif ($num == 2) {
			echo '<td width="19%"><a href="wx_albgen.php?view=Strip&img=1#start"><img width="96%" src="', $imgref, '1s.jpg" title="', $imgdescrip[0], '" alt="Photo 1..." /></a></td>';
			...
			echo '<td width="19%"><a href="wx_albgen.php?view=Strip&img=', $imgnum, '#start"><img width="96%" src="', $imgref, $imgnum, 's.jpg" title="', $imgdescrip[$imgnum - 1], '" alt="Photo', $imgnum, '..." /><a></td>';
		} elseif ($num == $imgnum) {
			...
			for ($i = $num - 2; $i <= $num + 2; $i++) {
				...
				echo '<td width="', $width, '%"><a href="wx_albgen.php?view=Strip&img=', $i, '#start"><img width="96%"';
				if ($num == $i) {
