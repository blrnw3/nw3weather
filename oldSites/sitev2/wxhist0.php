...
<head>
	<title>NW3 Weather - Old(v2) - Annual Pressure Reports</title>

	<meta name="description" content="Old v2 - Detailed historical annual barometric pressure report for <?php echo $year; ?> with monthly breakdown, from NW3 weather" />
...
<div id="main-copy">
	<div id="report">
		<h1>Pressure Reports (<?php echo $unitP; ?>)</h1>

<?php $self = 'wxhist10-5.php';
	include("wxrepgen.php"); 

get_baro_detail($year,$loc, $range, $season_start, $round); ?>

	</div>
		<p><b>Note:</b> All readings are rather dubious due to variable, sometimes large errors on the barometer, of approximately &plusmn;<?php echo conv(3,3,1); ?>.</p>
</div><!-- end main-copy -->
...
<?php 
function get_baro_detail ($year, $loc, $range, $season_start, $round) {
	global $SITE, $tempvalues, $temptype, $colors;
	global $show_today, $date, $time, $mnthname; 
	global $highbaro, $lowbaro, $unitP;
	...
	for ( $day = 0 ; $day < 31 ; $day++ ) {
		...
		for ($mnt = 0 ; $mnt < 12 ; $mnt++ ) {
			if($year == date("Y") && $mnt == date("m")-1 && $day == date("j")-1) {
				echo '<td class=" ' . ValueColor3(conv($lowbaro,3,0)).'">' . conv($lowbaro,3,0) .' </td>';
				echo '<td class=" ' . ValueColor2(conv($highbaro,3,0)).'">' . conv($highbaro,3,0) .' </td>';
			} 
			elseif ($maxdays[$mnt] < $day + 1 ) {
				...
				if ($put != "---" && $mnt != 0):
					echo '<td class=" ' . ValueColor3(conv($put,3,0)).'">' . conv($put,3,0) .' </td>';
				elseif ($put != "---" && $mnt == 0):
					echo '<td class=" ' . ValueColor(conv($put,3,0)).'">' . conv($put,3,0) .' </td>';
			...
			if ($put != "---" && $mnt != 11):
					echo '<td class=" ' . ValueColor2(conv($put,3,0)).'">' . conv($put,3,0) .' </td>';
				elseif ($put != "---" && $mnt == 11):
					echo '<td class=" ' . ValueColor(conv($put,3,0)).'">' . conv($put,3,0) .' </td>';
					else: echo '<td class="reportday" >' . "---" . '</td>';
			endif;
...
echo '<tr><th class="labels">&nbsp;</th>' ;
	...
	for ( $i = 0 ; $i < 12 ; $i++ ) {
		if ($tempmonth[$i][3] > 0) {
			echo '<td class=" ' . ValueColor(conv(min($raw[$i][0][3]),3,0)).'" colspan="2">' . conv(min($raw[$i][0][3]),3,0) . '</td>'; 
		}	else {
	...
	for ( $i = 0 ; $i < 12 ; $i++ ) {
		if ($tempmonth[$i][1] > 0) {
			echo '<td class=" ' . ValueColor(conv(max($raw[$i][0][2]),3,0)).'" colspan="2">' . conv(max($raw[$i][0][2]),3,0) .' </td>'; 
		} else {
			echo '<td class="reportttl" colspan="2">' . "---" . '</td>';
			}
	}
	...
	for ( $i = 0 ; $i < 12 ; $i++ ) {
		...
		if ( $mean != 0 ) {
			echo '<td class=" ' . ValueColor(conv($mean,3,0)).'" colspan="2">' . conv($mean,3,0). '</td>';
		} else {
			echo '<td class="reportttl" colspan="2" >' . "---" . '</td>';
			}
	} 
	...
	for ( $i = 0 ; $i < 12 ; $i++ ) { 
		if ($tempmonth[$i][3] > 0) {
			echo '<td class=" ' . ValueColor(conv(($tempmonth[$i][2] / $tempmonth[$i][3] ),3,0)).'" colspan="2">' . conv($tempmonth[$i][2] / $tempmonth[$i][3],3,0) . '</td>';
		}	else {
			echo '<td class="reportttl" colspan="2">' . "---" . '</td>';
			}
	}
	...
	for ( $i = 0 ; $i < 12 ; $i++ ) {
		if ($tempmonth[$i][1] > 0) {
		echo '<td class=" ' . ValueColor(conv(($tempmonth[$i][0] / $tempmonth[$i][1] ),3,0)).'" colspan="2">' . conv($tempmonth[$i][0] / $tempmonth[$i][1],3,0) . '</td>';
		} else {
			echo '<td class="reportttl" colspan="2" >' . "---" . '</td>';
			}
	}
...
$colorband_cols = ceil(($colors+1)/$colorband_rows); 
	...
	echo '<td class="level_'.($band+2).'" >&ge;'. sprintf($places,$tempvalues[$i-1]) . '</td>';
	echo '</tr></table>';
}
